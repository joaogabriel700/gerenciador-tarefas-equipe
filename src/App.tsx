...
export default function App() {
  ...
  // CONTROLADORES DOS FILTROS INTERATIVOS DOS NOVOS KPIS
  ...
  // =========================================================
  // MATEMÁTICA E LÓGICA DE EXECUÇÃO DOS NOVOS KPIS STRATÉGICOS
  // =========================================================

  // KPI 1: GARGALOS DE FLUXO (Retenção crítica nas colunas intermediárias)
  const listagemGargalos = tasks.filter(t => (t.status === 'Revisão' || t.status === 'Em Andamento') && t.priority === 'Alta');
  const qtdGargalos = listagemGargalos.length;

  // KPI 2: RISCO PREVENTIVO (Horas estimadas pendentes vs tempo restante real)
  const hojeTimestamp = Date.now();
  const listagemRiscoPrazo = tasks.filter(t => {
    if (t.status === 'Concluído' || !t.deadline) return false;
    const diferencaDias = Math.ceil((new Date(t.deadline).getTime() - hojeTimestamp) / (1000 * 60 * 60 * 24));
    return diferencaDias <= 2;
  });
  const qtdRiscoPrazo = listagemRiscoPrazo.length;

  // KPI 3: ÍNDICE DE DESBALANCEAMENTO DE ESFORÇO (Disparidade estatística do time)
  const cargasHorariasUsuarios = mockUsers.map(user => {
    const tarefasUser = tasks.filter(t => t.assigneeId === user.id && t.status !== 'Concluído');
    return tarefasUser.reduce((acc, t) => acc + ((t as any).estimatedHours || 0), 0);
  });
  const mediaHorasEquipe = cargasHorariasUsuarios.reduce((acc, h) => acc + h, 0) / mockUsers.length || 0;
  const somatorioDesviosAbsolutos = cargasHorariasUsuarios.reduce((acc, h) => acc + Math.abs(h - mediaHorasEquipe), 0);
  const indiceDesbalanceamento = Math.round(somatorioDesviosAbsolutos / mockUsers.length || 0);

  // Ordenação de capacidade para a aba Carga
  ...
  // Engine unificada de filtragem estrutural do Quadro
  const filtrarTarefas = (lista: Task[]) => {
    return lista
      ...
      .filter(t => !filtroGargaloAtivo || ((t.status === 'Revisão' || t.status === 'Em Andamento') && t.priority === 'Alta'))
      .filter(t => !filtroRiscoAtivo || (t.status !== 'Concluído' && t.deadline && Math.ceil((new Date(t.deadline).getTime() - hojeTimestamp) / (1000 * 60 * 60 * 24)) <= 2));
  };
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f3f4f6', padding: '32px', fontFamily: 'sans-serif', boxSizing: 'border-box' }}>
      <div style={{ maxWidth: '1400px', margin: '0 auto', width: '100%' }}>
        ...
        {/* CARDS DE METRICAS SUPERIORES ESTRATÉGICAS */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '24px', marginBottom: '32px' }}>
          
          {/* KPI 1: GARGALOS DE FLUXO */}
          <div 
            onClick={() => { setFiltroRiscoAtivo(false); setFiltroGargaloAtivo(!filtroGargaloAtivo); }}
          ...
          {/* KPI 2: RISCO PREVENTIVO DE PRAZO */}
          <div 
            onClick={() => { setFiltroGargaloAtivo(false); setFiltroRiscoAtivo(!filtroRiscoAtivo); }}
          ...
          {/* KPI 3: ÍNDICE DE DESBALANCEAMENTO DE ESFORÇO */}
        ...
        {modoVisao === 'quadro' && (
          <div style={{ backgroundColor: '#ffffff', padding: '16px', borderRadius: '8px', boxShadow: '0 1px 2px 0 rgba(0, 0, 0, 0.05)', marginBottom: '32px', display: 'flex', flexWrap: 'wrap', gap: '16px', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', width: '100%', maxWidth: '666px' }}>
              <select style={{ width: '100%', maxWidth: '300px', border: '1px solid #d1d5db', borderRadius: '6px', padding: '8px', backgroundColor: '#ffffff', outline: 'none' }} value={filtroPrioridade} onChange={e => setFiltroPrioridade(e.target.value)}>
                  ...
                  {filtrarTarefas(tasks.filter((task) => task.status === coluna)).map((task) => (
                    <div key={task.id} draggable onDragStart={(e) => handleDragStart(e, task.id)} onClick={() => abrirParaEditar(task)} style={{ backgroundColor: '#ffffff', padding: '16px', borderRadius: '6px', boxShadow: '0 1px 2px 0 rgba(0, 0, 0, 0.05)', borderLeft: '4px solid #3b82f6', cursor: 'pointer', transition: 'all 0.2s' }}>
                      ...
                      {task.blocksTasks && task.blocksTasks.length > 0 && (
                        <div style={{ marginTop: '8px', padding: '8px', backgroundColor: '#fee2e2', border: '1px solid #fecaca', borderRadius: '4px', fontSize: '0.75rem', color: '#dc2626', fontWeight: '500' }}>⚠️ Bloqueia {task.blocksTasks.length} tarefa(s)</div>
                      )}
                      <div style={{ marginTop: '8px', fontSize: '0.75rem', color: '#4b5563' }}><strong>Tempo Previsto:</strong> {(task as any).estimatedHours || 0}</div>
                      ...
                      {task.blocksTasks.map(blockId => {
                        ...
                        return (
                          <div key={blockedTask.id} style={{ position: 'relative', display: 'flex', alignItems: 'center', width: '100%' }}>
                            <div style={{ width: '16px', height: '2px', borderBottom: '2px dashed #9ca3af', position: 'absolute', left: '-32px', top: '50%' }}><div style={{ position: 'absolute', right: '0', top: '-4px', width: '6px', height: '6px', borderRight: '2px solid #9ca3af', borderBottom: '2px solid #9ca3af', transform: 'rotate(-45deg)' }}></div></div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '20px', width: '100%' }}>
              {cargasRankeadas.map(item => {
                ...
                if (mediaHorasEquipe > 0) {
                  ...
                  if (percentual > 1.25) {
                    faixaBorda = '#fca5a5'; faixaFundo = '#fef2f2'; faixaTexto = '#991b1b'; indicadorTexto = '🔴 Muita carga';
                  } else if (percentual < 0.75) {
                    faixaBorda = '#86efac'; faixaFundo = '#f0fdf4'; faixaTexto = '#14532d'; indicadorTexto = '🟢 Pouca carga';
                  } else {
                    faixaBorda = '#fde047'; faixaFundo = '#feffe0'; faixaTexto = '#713f12'; indicadorTexto = '🟡 Carga normal';
                  }
                }
                    ...
                    {tarefasFiltradasUser.length > 0 ? (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', overflowY: 'auto', flex: 1, paddingRight: '2px' }}>
                        {tarefasFiltradasUser.map(t => (
                          <div key={t.id} draggable onDragStart={(e) => handleDragStart(e, t.id)} onClick={() => abrirParaEditar(t)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px', backgroundColor: '#ffffff', border: '1px solid #e5e7eb', borderRadius: '6px', fontSize: '0.725rem', cursor: 'pointer', boxShadow: '0 1px 1px rgba(0,0,0,0.01)' }}>
                            <span style={{ fontWeight: '500', paddingRight: '6px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '120px' }} title={t.title}>{t.title}</span>
                            <span style={{ color: '#2563eb', fontWeight: '700', backgroundColor: '#eff6ff', padding: '1px 5px', borderRadius: '4px', fontSize: '0.6875rem', flexShrink: 0 }}>{(t as any).estimatedHours || 0}</span>
                          </div>
                        ))}
                      </div>
                    ) : (
          ...
          <div style={{ backgroundColor: '#ffffff', padding: '24px', borderRadius: '8px', width: '100%', maxWidth: '450px', maxHeight: '90vh', overflowY: 'auto', zIndex: 2147483648 }}>
            ...
            <form onSubmit={salvarTarefa} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div>
                ...
                <input type="number" min="0" style={{ marginTop: '4px', width: '100%', borderRadius: '6px', border: '1px solid #d1d5db', padding: '8px', outline: 'none', boxSizing: 'border-box' }} value={(novaTarefa as any).estimatedHours || 0} onChange={e => setNovaTarefa({ ...novaTarefa, estimatedHours: Number(e.target.value) } as any)} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '600', color: '#374151' }}>Bloqueia outras tarefas</label>
                <select multiple style={{ marginTop: '4px', width: '100%', borderRadius: '6px', border: '1px solid #d1d5db', padding: '8px', fontSize: '0.875rem', backgroundColor: '#ffffff', outline: 'none', height: '80px', boxSizing: 'border-box' }} value={novaTarefa.blocksTasks || []} onChange={e => { const selectedOptions = Array.from(e.target.selectedOptions, option => option.value); setNovaTarefa({ ...novaTarefa, blocksTasks: selectedOptions }); }}>{tasks.filter(t => t.id !== novaTarefa.id).map(taskOption => <option key={taskOption.id} value={taskOption.id}>{taskOption.title}</option>)}</select>
